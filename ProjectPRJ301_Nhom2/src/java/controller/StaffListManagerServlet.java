 ...
 * @author DELL
 */
@WebServlet(name="StaffListManagerServlet", urlPatterns={"/stafflist"})
public class StaffListManagerServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet StaffListManagerServlet</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet StaffListManagerServlet at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        ...
        List<Account> liststaff = cdb.getAccountStaff();
        ...
        liststaff = cdb.getListByPage(liststaff, start, end);

        // Đặt các thuộc tính vào request để chuyển đến JSP
        request.setAttribute("data", liststaff);
        request.setAttribute("page", page);

        request.getRequestDispatcher("./StaffManager/stafflist.jsp").forward(request, response);
    } 
