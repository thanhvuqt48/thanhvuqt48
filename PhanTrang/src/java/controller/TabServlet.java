...
public class TabServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        List<Product> list1 = pd.getByCategoryId(id);
        int page, numberElementsInPage = 6;
        int size = list1.size();
        int num = (size % numberElementsInPage == 0) ? (size / 6) : (size / 6 + 1);
        ...
        List<Product> list = pd.getListByPage(list1, start, end);
